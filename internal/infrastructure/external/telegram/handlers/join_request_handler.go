...
const hiMsg = "Привет, вы подали заявку на вступление в <a href=\"%s\">чат</a> дома по адресу Иловайская, 2 " +
	...
	"\n" +
	"Также у нашего чата и будущего дома есть <a href=\"https://ilovaiskaya2.homes\">небольшой сайт</a>, " +
	"там можно ознакомиться с правилами, найти полезные ссылки для будущего жильца и тп.\n" +
	"\n" +
...
func JoinRequestHandler(ctx tele.Context) error {
	...
	if _, err := ctx.Bot().Send(
		ctx.Sender(),
		fmt.Sprintf(hiMsg, config.InviteURL, config.OwnerNickname),
