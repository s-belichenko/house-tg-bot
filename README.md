...
- в личной переписке (доступны любому пользователю Telegram):
    ...
    - `/my_info` – показать пользователю его статус в домовом чате;
    - `/self_mute` – ограничить самого себя в домовом чате на заданное количество дней;
...
Уведомления для чата администраторов:

- при отправке пользователем заявки на вступление в домовой чат (заявки должны быть включены в настройках чата);
- при отправке жалобы на нарушение правил;
- при самоограничении пользователя;
- при одобрении заявки пользователя на вступление (если одобряется вручную, может не работать в некоторых чатах);
- при покидании пользователем чата (если одобряется вручную, может не работать в некоторых чатах).
...
- `JOIN_RULES` – Правила для вступления в чат (верификации как реально жильца).
